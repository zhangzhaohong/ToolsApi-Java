...
@Slf4j
@Service
public class EmailSenderService implements InitializingBean {
    ...
    public void scanRedis() {
        while (Boolean.TRUE.equals(runLoopFlag) || executorService.isTerminated()) {
            try {
                ...
                executorService.execute(() -> sendMail(mailDataContext));
            } catch (Exception e) {
                log.error("服务异常", e);
            }
        }
    ...
    private void sendMail(MailDataContext mailDataContext) {
        mailDataContext.startExec(emailService);
    }
